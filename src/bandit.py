...
def rununBiasedSimpleBandit(R,steps,runs,eps,init=0,alpha=0,randomWalkSigma = 0):
    rewards = []
    optAct  = []
    avg_value = []
    idx_R_max = [1 + a for a in np.where(R == np.max(R))[0]]
    for e in range(len(eps)):
        ...
        value   = [0 for r in R]
        for i in range(runs):
            reward, optimalActions, Q, o, beta = unBiased_simpleBandit(R,steps,eps[e],init,alpha,randomWalkSigma)
            avg_reward = [avg_reward[j] + reward[j]/float(runs) for j in range(steps)]
            opt_actCount = [float(a in idx_R_max) for a in optimalActions]
            percOptAct = [percOptAct[j] + opt_actCount[j]/float(runs) for j in range(steps)]
            value = [value[j] + Q[j]/float(runs) for j in range(len(Q))]
...
def simpleBandit_UCB(R,steps,eps,c,init = 0,alpha=0,randomWalkSigma = 0):
    k = len(R)
    Q = []
    N = []
    for i in range(k):
    ...
    optimalActions = []
    for i in range(steps):
        obj = [Q[j] + c*np.sqrt(np.log(i+1)/max(1E-6,float(N[j]))) for j in range(k)]
        prob = np.random.uniform(0,1,1)[0]
        if prob >= eps:
            idx_obj_max = np.where(obj == np.max(obj))[0]
            a = int(np.random.uniform(idx_obj_max[0],idx_obj_max[-1]+1,1)[0])
        else:
            a = int(np.random.uniform(0,k,1)[0])
        Ri = Gaussian(R[a],1.0)[0]
        reward += [Ri]
...
def runSimpleBandit_UCB(R,steps,runs,eps,c,init=0,alpha=0,randomWalkSigma = 0):
    rewards = []
    optAct  = []
    avg_value = []
    idx_R_max = [1 + a for a in np.where(R == np.max(R))[0]]
    for e in range(len(eps)):
        ...
        value   = [0 for r in R]
        for i in range(runs):
            reward, optimalActions, Q = simpleBandit_UCB(R,steps,eps[e],c,init,alpha,randomWalkSigma)
            avg_reward = [avg_reward[j] + reward[j]/float(runs) for j in range(steps)]
            opt_actCount = [float(a in idx_R_max) for a in optimalActions]
            percOptAct = [percOptAct[j] + opt_actCount[j]/float(runs) for j in range(steps)]
            value = [value[j] + Q[j]/float(runs) for j in range(len(Q))]
